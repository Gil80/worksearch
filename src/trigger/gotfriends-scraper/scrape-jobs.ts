...
import { parseListingPage, parseListingCards, matchesSearchCriteria, type JobListing } from "./parse-listing.js";
import {
  getExistingJobIds,
  appendJobListings,
  ensureSheetHeaders,
...
export const scrapeGotFriendsJobs = task({
  id: "scrape-gotfriends-jobs",
  run: async () => {
    ...
    try {
      ...
      // Scrape all pages (limit to 2 pages for now to test memory)
